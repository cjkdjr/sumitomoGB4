    	...
    	if (ret > 0)
    	{
            ...
            if((p=strstr(p,"+CMGL:"))!=NULL)
            {
                ...
                if(p!=NULL)
                {
                    ...
                    if((0 == memcmp(pMsg.TPA, CenterSendNum,strlen(CenterSendNum)))
                        || (0 == memcmp(pMsg.TPA, CenterSIM1Number,strlen(CenterSIM1Number)))
    		            || (0 == memcmp(pMsg.TPA, CenterSIM2Number,strlen(CenterSIM2Number)))
    		            || (0 == memcmp(pMsg.TPA, GSM_M3M_CENTER1,strlen(GSM_M3M_CENTER1)))
...
//                        printf_gsm("\n");
                        ...
                        if(pMsg.TP_UD[0] == 0X7E){//中心设置消息，做分流处理
                        	...
                        	RecData.identifNum = pMsg.TP_UD[num++];
                        	RecData.identifNum = (RecData.identifNum<<8)|(pMsg.TP_UD[num++]);
                        	RecData.identifNum = (RecData.identifNum<<8)|(pMsg.TP_UD[num++]);
                        	for(j=0;j<len-num;j++)
                        	{
...
//	unsigned char DataCheck=0;
	...
	if(length > 0)
	{
		...
		RecData.identifNum = data[index++];
		RecData.identifNum = (RecData.identifNum<<8)|data[index++];
		RecData.identifNum = (RecData.identifNum<<8)|data[index++];
