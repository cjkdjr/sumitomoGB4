...
int CANA_fd = -1, CANB_fd = -1, CANC_fd = -1;
...
int CAN_Open() {
	...
	struct ifreq ifr;
	int j;
	gpio_SetModeValue("43" , "high");  //can2(CANA)
	gpio_SetModeValue("141", "high");  //	CAN0_STB:PE13   (5-1)*32 +13=141
	gpio_SetModeValue("256", "high");  //	CAN1_STB:PI0   (9-1)*32 +0=256
//    printf("can_set_bitrate:CAN0:%d,CAN1:%d\n",SYS_SET.SET_CAN0_BAUD,SYS_SET.SET_CAN1_BAUD);
	if ((CANA_fd < 0) || (CANB_fd < 0 || CANC_fd < 0)) {
		can_set_bitrate_samplepoint("can0", 250000, 750);
		can_do_start("can0");
		can_set_bitrate_samplepoint("can1", 250000, 750);
		can_do_start("can1");
		can_set_bitrate_samplepoint("can2", 500000, 750);
		can_do_start("can2");
		...
		for (j = 0; j <= 2; j++) {
			...
			s[j] = socket(PF_CAN, SOCK_RAW, CAN_RAW);
			if (s[j] < 0) {
				printf("socket error\n");
				return -1;
			}
			...
			//set can socket timeout
			struct timeval timeout;
			timeout.tv_sec = 5;  //秒
			timeout.tv_usec = 0;  //微秒
			if (setsockopt(s[j], SOL_SOCKET, SO_RCVTIMEO | SO_SNDTIMEO, &timeout, sizeof(timeout)) == -1) {
...
//			fcntl(s[j], F_SETFL, fcntl(s[j], F_GETFL, 0) | O_NONBLOCK);  //设置非阻塞
		}
		CANB_fd = s[0];
		CANC_fd = s[1];
		CANA_fd = s[2];
		printf("[  CAN_Open]:CANA_fd=%d CANB_fd=%d  CANC_fd=%d\n", CANA_fd, CANB_fd, CANC_fd);
	}
	return 0;
}
int CAN_close() {
...
/********************************************
 * Function:socket can read data
 * Input:1.fd - can net fd
 *  2.canrxbuf - buf receive can data
 *  3.canbufnum - number of canrxbuf
...
int can_write(int fd, struct can_frame* CanTxBuf) {

	struct can_frame frame_tx;
	int i = 0, nbytes = 0;
	frame_tx.can_id = CAN_EFF_FLAG | CanTxBuf->can_id;
	frame_tx.can_dlc = CanTxBuf->can_dlc;
	for (i = 0; i < CanTxBuf->can_dlc; i++) {
		frame_tx.data[i] = CanTxBuf->data[i];
	}
	nbytes = write(fd, &frame_tx, sizeof(struct can_frame));
//	if(nbytes <= 0)
//	{
//		perror("can write");
//	}
 ...
 */
enum {
	IFLA_CAN_UNSPEC, IFLA_CAN_BITTIMING, IFLA_CAN_BITTIMING_CONST, IFLA_CAN_CLOCK, IFLA_CAN_STATE, IFLA_CAN_CTRLMODE, IFLA_CAN_RESTART_MS, IFLA_CAN_RESTART, IFLA_CAN_BERR_COUNTER, __IFLA_CAN_MAX
};
...
static int do_get_nl_link(int fd, __u8 acquire, const char *name, void *res) {
	...
	struct msghdr msg = { .msg_name = (void *) &peer, .msg_namelen = sizeof(peer), .msg_iov = &iov, .msg_iovlen = 1, .msg_control = &cbuf, .msg_controllen = sizeof(cbuf), .msg_flags = 0, };
